...
def recent_server_sleep(state, i, v), do:
  Map.put(state, :recent_server_sleep, Map.put(state.recent_server_sleep, i, v))

def total_server_sleep(state, i, v), do:
  Map.put(state, :total_server_sleep, Map.put(state.total_server_sleep, i, v))
...
def start(config) do
  state = %{
    ...
    recent_server_sleep:    Map.new,
    total_server_sleep: Map.new
  }
...
def next(config, state) do
  ...
  { :SERVER_SLEEP, server_num, time_out } ->
    state = Monitor.recent_server_sleep(state, server_num, time_out)
    value = Map.get(state.total_server_sleep, server_num, 0)
    state = Monitor.total_server_sleep(state, server_num, value + time_out)
    Monitor.next(config, state)
  ...
  { :PRINT } ->
    ...
    if config.debug_level == 0 do
      ...
      sorted = state.recent_server_sleep |> Map.to_list |> List.keysort(0)
      IO.puts "time = #{clock}      recent server timeouts = #{inspect sorted}"

      sorted = state.total_server_sleep |> Map.to_list |> List.keysort(0)
      IO.puts "time = #{clock}      total server timeouts = #{inspect sorted}"
